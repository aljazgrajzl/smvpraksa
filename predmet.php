...
$predmet_id = $_GET['id'] ?? 0;

// Pridobi podatke o predmetu
$predmet_query = "SELECT * FROM vsi_predmeti WHERE id = ?";
$stmt = $conn->prepare($predmet_query);
$stmt->bind_param("i", $predmet_id);
$stmt->execute();
$predmet = $stmt->get_result()->fetch_assoc();

// Pridobi oddane naloge za tega dijaka in predmet
$oddane_naloge = [];
if ($_SESSION['user_type'] == 'dijak') {
    $naloge_query = "SELECT * FROM oddane_naloge WHERE dijak_id = ? AND predmet = ? ORDER BY datum_oddaje DESC";
    $stmt = $conn->prepare($naloge_query);
    $stmt->bind_param("is", $_SESSION['user_id'], $predmet['ime_predmeta']);
    $stmt->execute();
    $oddane_naloge = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
}
...
<head>
    ...
    <title><?php echo $predmet['ime_predmeta']; ?> - E-Šola</title>
    <!-- Bootstrap 5 CSS -->
...
<nav class="navbar navbar-expand-lg navbar-dark">
    ...
    <section class="subject-header">
        <div class="container">
            <h1 class="display-4 fw-bold"><?php echo $predmet['ime_predmeta']; ?></h1>
            <p class="lead"><?php echo $_SESSION['razred'] ?? '4. letnik'; ?> - Srednja šola</p>
            <div class="d-flex flex-wrap align-items-center mt-3">
                <div class="me-4">
                    <span class="badge bg-light text-dark p-2"><i class="bi bi-person-check me-1"></i> Profesor</span>
                </div>
                <div class="me-4">
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            <div class="card-body text-center">
                                ...
                                <h5 class="card-title mt-2">Računanje v dvojiškem številskem sistemu.</h5>
